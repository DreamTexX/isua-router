...
console.log(format({
  ...
  results: {
    "/companies": sync(
      1e6,
      () => assert(root.lookup("/companies")[0] === "01"),
    ),
    "/companies/:id": sync(
      1e6,
      () => assert(root.lookup("/companies/cool-id-1234")[0] === "02"),
    ),
    "/companies/:id/users": sync(
      1e6,
      () => assert(root.lookup("/companies/cool-id-1234/users")[0] === "03"),
    ),
    "/companies/:id/users/:id": sync(
      1e6,
      () =>
        assert(
          root.lookup("/companies/cool-id-1234/users/12345-lol")[0] === "04",
        ),
    ),
    "/users": sync(1e6, () => assert(root.lookup("/users")[0] === "05")),
    "/users/:id": sync(
      1e6,
      () => assert(root.lookup("/users/cool-id-1234")[0] === "06"),
    ),
    "/users/:id/*file": sync(
      1e6,
      () =>
        assert(root.lookup("/users/cool-id-1234/some/file.png")[0] === "07"),
    ),
  },
}));
